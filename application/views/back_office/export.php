	...
	<div class="main-wrappper">
    	<div style="width:100%;" id="filter-div">
        	<table cellspacing="0" cellpadding="0">
            	<form id="generate_numbers_form" action="/back_office/export">
                    <tr>
        ...
        <?php if(isset($numbers) && count($numbers)){?>
                    ...
                    <tbody>
                        ...
                        foreach($numbers as $key  => $number){
							...
							echo "<tr class='".$class."'>";
								...
								$status = (($number["back_ofice_status"] == "")?"-":(($number["back_ofice_status"] == "1" )?"Valid":"Invalid"));
								echo "<td>".$status."</td>";
							echo "</tr>";
                        }
                        ?>
            ...
            <div style="width:100%;margin-top:50px;" id="filter-div-bottom">
                <table cellspacing="0" cellpadding="0" style="width:100%;">
                    <tr>
                        <td colspan="6"><select id="export_option" style="float:left;margin-right:15px;"><option value="all">All</option><option value="unique">Exclude Duplicates</option></select><?php /*?><input type="button" class="export_no excel" value="Export Excel" style="float:left;"/><?php */?> <input type="button" class="export_no csv" value="Export CSV" style="float:left;"/> <input type="button" id="validate_no" value="Validate" style="float:right;"/></td>
                    </tr>
                </table>
            </div>
        <?php }?>
...
<script>
	...
	$(document).ready(function(){
		...
		$(".export_no").click(function(){
			...
			//var back_office_status = "";
			$(selectClass).each(function(index, element){
				/*if($(element).find('td:eq(3)').text() == ""){
					back_office_status = "";
				}
				else if($(element).find('td:eq(3)').text().toLowerCase() == "invalid"){
					back_office_status = "0";
				}
				else if($(element).find('td:eq(3)').text().toLowerCase() == "valid"){
					back_office_status = "1";
				}*/
				TableData[iteration] = {
					"number" : 	     $(element).find('td:eq(1)').text(),
					"emirate_id" :   $(element).find('td:eq(2)').text(),
					"back_ofice_status" : $(element).find('td:eq(3)').text(),
					"export_token" : token
				}
		...
		$(document).ajaxStop(function() {
			...
			//return;
			window.location = "/optimize/"+export_target+"/?token="+token+"&status=completed&update=true";
			/*$.ajax({
				type: "POST",
		...
		$("#generate_no").click(function(e){
			...
			window.location = "/back-office/generate_numbers/?"+$("#generate_numbers_form").serialize();
