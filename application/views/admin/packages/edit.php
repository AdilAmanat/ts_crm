...
$package_name = array(
	...
	"id"   => "package_name",
	"type" => "text",
	"value" => (isset($_REQUEST["package_name"]))?$_REQUEST["package_name"]:(isset($package["package_name"])?$package["package_name"]:""),
	"class" => "form-control",
	"required" => "required",
	"maxlength" => "50"
);
$package_price = array(
	...
	"id"   => "package_price",
	"type" => "text",
	"value" => (isset($_REQUEST["package_price"]))?$_REQUEST["package_price"]:(isset($package["package_price"])?$package["package_price"]:""),
	"class" => "form-control",
	"required" => "required"
);
                	...
                	<form class="card" id="add_package_form" method="post">
                        ...
                        <div class="card-body">
                            <div class="col-md-6 append-to-wrapper">
                                ...
                                <?php
								foreach($package["details"] as $key => $row){
									$package_duration = array(
										"name" => "package_duration[]",
										"id"   => "package_duration",
										"type" => "text",
										"value" => $row["package_duration"],
										"class" => "form-control",
										"required" => "required"
									);
									$package_description = array(
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="package_duration[]" id="package_duration" class="form-control custom-select" required>
                                        <option value="">Please select</option>
                                        <?php
                                        $val = (isset($_REQUEST["package_duration"]))?$_REQUEST["package_duration"]:(isset($row["package_duration"])?$row["package_duration"]:"");
                                        foreach($durations as $duration){
                                        ?>
                                        <option value="<?php echo $duration["duration"];?>" <?php echo (strtolower($val) == strtolower($duration["duration"]))?'selected="selected"':"";?>><?php echo ucfirst($duration["duration"]);?></option>
                                        <?php
                                        }
                                        ?>                           
                                    </select>
