...
class Teamlead_model extends CRM_Model {
	...
	public function get_assigned_numbers_by_series($export_token, $timestamp, $assgined_by){
		...
		if($assgined_by == "tsa"){
			$this->db->select("tsa_status.status as status_name");
	...
	public function get_numbers_assigned_by_tsa(){
		$numbers = array();
		$user_id = $this->session->userdata('user_id');
		$this->db->select("assigned_numbers.*, count('*') as total_numbers, numbers.export_token, min(cast(numbers.number as unsigned)) as start_number, max(cast(numbers.number as unsigned)) as end_number, tsa_status.status as status_name, users.first_name as assigned_by_first_name, users.last_name as assigned_by_last_name")->from("assigned_numbers");
	...
	public function get_numbers_assigned_by_tl_csr(){
		$numbers = array();
		$user_id = $this->session->userdata('user_id');
	...
	public function get_unassigned_numbers_from_series_without_feedback($export_token, $teamlead_id){
		$numbers = array();
		$this->db->select("numbers.*, numbers.id as no_id")->from("numbers");
		
		if (!empty($export_token)) {
			$this->db->where("(assigned_to is null or assigned_to = '')", null, false);
			$this->db->where("export_token", $export_token);
		}
		elseif (!empty($teamlead_id)) {
			$this->db->where("assigned_to", $teamlead_id);
			$this->db->where("(is_assinged_to_csr is null or is_assinged_to_csr = '')", null, false);
		}
